...
fn resolve_python(repo_root: &Path) -> PathBuf {
    if let Ok(p) = std::env::var("COZMO_PYTHON") {
...
impl BackendLauncher {
    pub fn new(config: BackendConfig) -> Self {
        let command = if let Ok(exe) = std::env::var("COZMO_BACKEND_BIN") {
    ...
    pub fn start(&self) -> Result<(), String> {
        ...
        *self.child.lock().unwrap() = Some(child);
        println!(
            "[cozmo-desktop] backend spawned: {} {} (pid {pid})",
            program.display(),
            args.join(" ")
        );
        Ok(())
    ...
    pub fn stop(&self) {
        ...
        if let Some(_child) = child {
            ...
            println!("[cozmo-desktop] backend stopped (pid {})", pid.unwrap());
        }
    }
}