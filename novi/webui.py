"""
WebUI Backend builder for Novi.

Uses NoviContext to provide a single source of truth for models, memory,
...
class WebUIBackend:
    ...
    def build_backend(self) -> dict:
        ...
        # Capability registry — the composition root's instance (the same one
        ...
        # ctx.orchestrator.task_store, so the coordinator, continuation, and
        # ExecutionHistory bookkeeping must all observe the SAME store.
        ...
        def _reload_models(path, value, previous):
            ...
            try:
                ...
                print(f"[novi] model refresh failed: {e}")
        ...
        def _safe_telegram_refresh(lifecycle, path, value, previous):
            try:
                ...
                print(f"[novi] Telegram config refresh failed: {e}")
