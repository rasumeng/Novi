...
Composes Novi's existing ReAct agent loop into an explicit workflow:
...
from .state import CodingState, append_error, should_stop, emit_event
...
class CodingGraph:
    ...
    Args:
        model: Already-constructed LangChain chat model (Runnable). Used only
            ...
            direct use). Novi resolved and built it upstream — never
            constructed here.
    ...
    def _node_verify(self, state: dict) -> dict:
        ...
        _stream_event(state, {"phase": "verifying"})
        try:
