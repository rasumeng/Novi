"""LangGraph workflow state for Novi's agentic graphs (Phase 7 Stage 3).
...
persistence — those remain Novi-owned authorities.
...
class AgentStateBase(TypedDict, total=False):
    ...
    emit: Any                # Callable[[tuple], None] live event sink (runtime-owned)


def emit_event(state: dict, item: tuple) -> None:
    """Push one stream item to the runtime's LIVE channel, if wired.

    Nodes keep appending to their ``events`` / ``stream_events`` buffers for
    parity/evaluation harnesses; this is the ADDITIVE live path so the WebUI
    observes tokens/phases/tool activity DURING graph execution instead of in
    an end-of-run replay. Never raises — a broken sink must not fail a run.
    """
    sink = state.get("emit")
    if sink is None:
        return
...
class ResearchState(AgentStateBase, total=False):
    ...
    answer_streamed: bool    # True once synthesis has streamed answer tokens
...
class RuntimeState(AgentStateBase, total=False):
    ...
    - ``model`` is the runnable Novi bound for THIS run — graphs never
...
class CodingState(AgentStateBase, total=False):
    ...
    :class:`~novi.graphs.coding_intel.VerificationReport` results;
