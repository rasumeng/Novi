...
def _ensure_default_config():
    ...
    if not CONFIG_PATH.exists():
        ...
        logging.getLogger("novi.cli").info(
...
def interactive_session(ctx, initial_query: str | None = None):
    ...
    if initial_query:
        _safe_print(f"\nNovi: {session.run(initial_query)}\n")
    while True:
        try:
            user = input("\nYou: ")
...
def run_telegram(ctx):
    ...
    print("Novi Telegram bot started. Press Ctrl+C to stop.")
    bot.run()


def main():
    parser = argparse.ArgumentParser("novi")
    sub = parser.add_subparsers(dest="command")

    sub.add_parser("init", help="Generate ~/.novi/config.toml")
    sub.add_parser("telegram", help="Run Novi as Telegram bot")
    ...
    rebuild_parser.add_argument("--home", default=None, help="Profile directory to rebuild (default ~/.novi)")
    ...
    if args.command == "init":
        ...
        from .services import NoviContext
        ctx = NoviContext()
        run_telegram(ctx)
        ...
        try:
            print(f"Novi WebUI at http://{args.host}:{args.port}")
        ...
        home = args.home or str(app_home())
        report = rebuild(home)
        ...
        try:
            print(f"Novi WebUI at http://127.0.0.1:8765")
