...
Reads the legacy flat ``novi_knowledge`` LanceDB table written by Phase C's
...
Run once per profile:
    python -m novi.brain.storage.migrations --persist-dir ~/.novi/brain
"""
...
log = logging.getLogger("novi.brain.storage.migrations")

FLAT_TABLE = "novi_knowledge"
# Tables written before the Novi rebrand carried the old product name.
LEGACY_FLAT_TABLES = ("cozmo_knowledge",)
...
def migrate(persist_dir: str | Path, embed_model: str | EmbeddingService | None = None) -> dict:
    """Re-embed flat novi_knowledge rows into typed knowledge_items.
    ...
    if flat_table not in names:
        flat_table = next(
            (t for t in LEGACY_FLAT_TABLES if t in names), None
        )
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        "--persist-dir",
        type=str,
        default=str(Path.home() / ".novi" / "brain"),
        help="Brain persist directory (default ~/.novi/brain)",
    )
    parser.add_argument(
