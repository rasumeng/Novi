...
export const PromptInput = forwardRef<PromptInputHandle, Props>(function PromptInput({
  ...
  const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    ...
    // Files only. Novi's own `disable_drag_drop_handler()` in the Tauri shell
  ...
  return (
    ...
    >
      ...
      <textarea
        ...
        onPaste={handlePaste}
        placeholder={disabled ? 'Connecting to Novi…' : 'Message Novi...'}
        aria-label={generating ? 'Message Novi (stop to interrupt)' : 'Message Novi'}
        rows={1}
        disabled={disabled}
