...
Mirrors the JobStore pattern (``novi/jobs/persistence.py``): one JSON file per
task under ``~/.novi/taskstore/``. Thread-safe behind a lock. This module owns
...
log = logging.getLogger("novi.orchestrator.tasks")

from ..paths import home as app_home

TASKS_DIR = app_home() / "taskstore"
