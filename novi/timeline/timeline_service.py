...
def _entry_for(event) -> dict:
    ...
    if t == KNOWLEDGE_EXTRACTED:
        ...
        if not detail:
            detail = "Novi captured new details from a conversation."
        return {
            "kind": t,
    ...
    if t == KNOWLEDGE_PROMOTED:
        ...
        detail = f"Novi {', '.join(parts)} knowledge item{'s' if sum([promotions, corroborated, superseded]) != 1 else ''}."
        return {
            "kind": t,
...
def build_knowledge_overview(brain) -> dict:
    """User-shaped "What Novi knows" view from the Brain's projection.
