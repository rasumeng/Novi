"""
NoviContext — composition root / dependency container.
...
class NoviContext:
    ...
    def __init__(self, cfg: dict | None = None):
        ...
        # Live-config subscription guard. The subscription must be idempotent
        # and MUST NOT be tied to lazy seeding: the WebUI composition path
        # constructs NoviContext with an already-materialized framework
        # snapshot (get_backend -> WebUIBackend(cfg)), which left _cfg
        # non-None forever and silently skipped ``on_any`` — services then
        # held the startup snapshot and model/Settings changes only took
        # effect after a process restart.
        self._cfg_subscribed: bool = False
    ...
    def config(self) -> dict:
        ...
        if not self._cfg_subscribed:
            # Track live configuration: without this, services holding
            # ``self.config`` (ModelService et al.) keep a stale startup
            # snapshot and Settings changes never reach them.
            self._cfg_subscribed = True
            get_configuration().on_any(self._on_config_change)
        return self._cfg
    ...
    def memory(self):
        ...
        if self._memory is None:
            ...
            self._memory = MemoryManager(
                self.simple_llm,
                persist_dir=str(app_home() / "memory"),
    ...
    def recover_once(self, bus=None) -> list[dict]:
        """Idempotent recovery sweep, run at most once per NoviContext.
