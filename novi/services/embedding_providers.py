...
class OllamaEmbeddingProvider(EmbeddingProvider):
    ...
    Uses the canonical ``DEFAULT_EMBEDDING_MODEL`` when none is configured
    (startup provisioning guarantees it is installed). No HF Hub access, no
    model download — whatever model is configured must already exist in
    Ollama.
    """
    ...
    def __init__(self, config: dict | None = None):
        super().__init__(config)
        from ..configuration.install import DEFAULT_EMBEDDING_MODEL
        ...
        self._model = (
            (self._config.get("embedding", {}).get("model", "") or "").strip()
