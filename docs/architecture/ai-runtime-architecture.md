...
Durable spec for Novi's model runtime and LangGraph workflow composition as
...
| Job/Checkpoint/ContinuationService remain the resume authority | `novi.services.job_lifecycle`, `novi.services.continuation` |
| Tool permissions/risk stay inside ToolExecutor | `novi.runtime.tool_executor` |
...
ModelService                               novi/models/service.py
        ...
        ▼
ModelRuntime                               novi/runtime/models/factory.py
        ...
        ▼
create_provider(name)                      novi/providers/base.py
...
* `ChatOllama` / `ChatOpenAI` are constructed in exactly one place:
  `novi/providers/base.py`. Any new provider joins via
  `PROVIDER_REGISTRY`, not a new construction site.
* The runtime binds through exactly one method:
  `NoviRuntime._bind_runnable` (`novi/runtime/runtime.py`). Recovery and
...
* **Selection (write):** `apply_selection()`
  (`novi/configuration/resolver.py`) is the only code path that persists a
...
```
<entry surface>
   └─> ExecutionCoordinator.run_stream          novi/services/execution.py
         └─> Orchestrator.plan                  novi/orchestrator/
               │   IntentDetector / ComplexityEstimator / EvidenceDetector /
               │   RetrievalPolicy  (analysis computed ONCE, upstream)
               └─> NoviRuntime.run_stream      novi/runtime/runtime.py
...
Graph wiring is injection-based and opt-out-able: `NoviRuntime(research_graph=
...
Module: `novi/graphs/research_graph.py` (Phase 8B upgrade). State:
`ResearchState` (`novi/graphs/state.py`). Deterministic helpers:
`novi/graphs/research_intel.py`.
...
Module: `novi/graphs/coding_graph.py` (Phase 8C upgrade). State:
`CodingState`. Verification contracts: `novi/graphs/coding_intel.py`.
...
plan steps (PlannerEngine templates, deterministic) or unplanned single loop
   └─> run_react_attempt  (novi/runtime/react_attempt.py — sole generic
...
This path is deliberately retained: it serves bare `NoviRuntime()`
...
|---|---|---|
| CLI / background / Telegram / scheduler / task queue | `NoviContext.create_runtime` (`novi/services/context.py`) | yes (defaults constructed there) |
| WebUI sessions | `build_runtime` (`novi/webui_server.py`) | **yes since Stage 3E** — built once, cached on the shared backend dict (`b["research_graph"]` / `b["coding_graph"]`) |
| Evaluation harness | bare `NoviRuntime()` (`novi/evaluation/drivers.py`) | no (intentional baseline) |
...
* **Canonical conversation identity:** Brain (`novi/brain/`). Runtime turns
  are observed via `brain.observe(Turn(...))`.
* **Durable job state:** `JobLifecycle` (`novi/services/job_lifecycle.py`)
...
* **Resume candidates:** `ContinuationService`
  (`novi/services/continuation.py`) surfaces resumable work to CLI/WebUI.
...
`novi/runtime/tool_executor.py` is the single execution pipeline: extraction
...
* **`MCPManager`** (`novi/runtime/providers/mcp.py`) — facade over the M5.5
...
* **`SimpleLLM`** (`novi/services/simple_llm.py`) — auxiliary invoke seam for
  intent classification / evidence grounding / compaction. Propagates
  `ModelUnavailableError`; never substitutes.
* **`novi/ollama.py`** — Ollama *process* management (start/stop/health),
  not a model client shim.
* **Desktop vision** (`novi/tools/desktop.py`) — uses the verbatim selected
...
| Bare `NoviRuntime()` in evaluation driver | Intentional measurement baseline; wiring graphs there would silently change benchmark semantics. |
...
|---|---|---|
| Legacy ReAct branch in `run_stream` (planned sequential + unplanned) | **RETAIN** | It IS the `workflow_engine="legacy"` escape hatch surface, the bare-`NoviRuntime()` evaluation baseline, and the fallback when graphs are not wired. Removing it is Phase-5-final work, gated on the conditions below. |
| `_run_agent_loop` | **RETIRED (Phase 9C)** | No production definition or caller remains. The loop body lives verbatim in `novi/runtime/react_attempt.py::run_react_attempt` — the sole generic ReAct executor, driven directly by the sequential planned-step path, the unplanned path, and CodingGraph's `run_loop`. Guards: `tests/test_react_attempt_parity.py` (retirement architecture tests). |
...
The single evaluation framework lives in `novi/evaluation`. Phase 8E extends
it additively — there is no second harness anywhere in the repo.

```bash
# Offline orchestrator decision baseline (no model required)
python -m novi.evaluation analyze   [--dataset tests/regression_corpus.json]
...
python -m novi.evaluation research  [--dataset tests/research_corpus.json] [--save out.json]
...
python -m novi.evaluation coding    [--dataset tests/coding_corpus.json] [--save out.json]

# Regression: compare two saved MetricSet snapshots.
python -m novi.evaluation compare BASELINE.json CANDIDATE.json
```
...
**Regression thresholds** (`novi/evaluation/regression.py`). Comparison is
