# Cozmo Memory, Context, RAG, Document Ingestion & LangGraph Integration — Architecture Audit

**Scope:** Read-only architectural audit of Cozmo's memory/context/RAG/document-ingestion systems, and their relationship to LangGraph/LangChain.
...
|---|---|---|
| LangGraph orchestrates Cozmo | **False.** No `StateGraph`, `compile`, `create_react_agent` anywhere in the repo. | repo-wide `rg` scan; `STAGE0-AI-RUNTIME-AUDIT.md:136` |
| LangChain powers models | **True**, narrowly. `langchain_core.messages` (HumanMessage/SystemMessage/AIMessage), `ChatOllama`/`ChatOpenAI` via `cozmo.providers`, `bind_tools`. | `cozmo/providers/base.py`; `cozmo/runtime/models/factory.py` |
| LangChain "structured tools" | **True.** `StructuredTool` used for the tool loop. | `cozmo/runtime/tool_executor.py` |
| LangGraph dependency declared | **True.** `pyproject.toml:30`. | pyproject |

**Conclusion:** LangGraph is a placeholder dependency. The agentic loop (`CozmoRuntime.run_stream`), planning, retrieval dispatch, and memory write/read are all **hand-rolled orchestration** inside `cozmo/runtime/runtime.py`, `cozmo/orchestrator/orchestrator.py`, `cozmo/runtime/retrieval.py`.

### 1.2 Memory system — two parallel architectures

**A) Legacy flat memory (`cozmo/memory/manager.py`, `MemoryManager`)** — STILL ALIVE as fallback.
- Short-term turn buffer (≤10 pairs) → every 5 turns LLM summary → keyword classification (`prefer`/`project`/`fact`) → LanceDB `cozmo_memories` table (`~/.cozmo/memory/lancedb/`).
...
**B) Brain knowledge system (`cozmo/brain/`) — Phase A–F implemented.**
- `Brain` facade: `observe / recall / learn / resolve / reflect / inspect_memory / correct_memory / project_context` (`cozmo/brain/brain.py`).
...
- User trust surface: `inspect_memory()` (what Cozmo remembers + edges), `correct_memory()` (supersede/demote/archive, append-only, supersedes edges).

### 1.3 RAG / retrieval orchestration (`cozmo/runtime/retrieval.py`)
...
- Source adapters (`cozmo/runtime/sources/`): `MemoryRetrievalSource` (wraps **Brain.recall** when Brain wired, else flat MemoryManager), `KnowledgeRetrievalSource` (file KB index), `ProjectRetrievalSource` (code index), `WebRetrievalSource` (searxng), `ScenarioRetrievalSource`, `IdentityRetrievalSource`, `FileRetrievalSource` (**NoOp stub**).
...
- **Evidence pipeline** (`cozmo/evidence/`, web-evidence only): `EvidenceProcessor` (rank→facts→conflicts→confidence→compress) used ONLY in `cozmo/evaluation/evidence_ab.py`; the runtime `execute_search` produces raw `EvidenceBundle` without the processor. FactExtractor is web-only, not chat-capable.

### 1.4 Document / project ingestion (`cozmo/code_indexer.py`, `cozmo/memory/knowledge_index.py`)

- **ProjectIndex** (`~/.cozmo/project_index/<project-sha1>/`, table `project_index`): **whole-file embedding — no chunking**, no mtime re-index guard, no dedup on re-index. `.cozmo/project_index/` has leftover `chroma.sqlite3` (legacy migration artifact).
- **KnowledgeIndex** (`~/.cozmo/knowledge_index/lancedb/`, table `knowledge_index`): scans `workspace.knowledge` dir (default `~/.cozmo/knowledge`, currently configured to `D:\Projects\Cozmo\knowledge` — **which is EMPTY**). OKF Markdown (YAML frontmatter: type/title/tags/timestamp), overlapping paragraph chunking (1000 chars, 150 overlap), deterministic ids `<rel>::<chunk>` so re-index replaces rows, cross-encoder rerank (sentence-transformers), hybrid vector+keyword.
- **Config:** `embedding.backend=ollama`, `embedding.model=nomic-embed-text`, `dimension=768` (defaults; sentence-transformers backend available). `EmbeddingService`/`RerankerService` are facades over a provider registry (`cozmo/services/embedding.py`, `embedding_providers.py`). Config default has `model=""` — resolved via discovery/selection, never silently substituted. `memory/rebuild.py` drops vector stores when embedding backend changes; brain SQLite persists.
...
2. **WebUI markdown chats** `~/.cozmo/chats/<id>.md` + `index.json` — UI-owned, runtime-unaware, raw text, no frontmatter.
3. **Brain `ConversationStore`** (`~/.cozmo/brain/` SQLite) — raw turns + tool outputs, canonical Brain identity (`conv-...`).
4. **Legacy flat memory summaries** — lossy 5-turn summaries (only durable artifact in the old system).
Plus **TimelineStore** (`~/.cozmo/timeline/timeline.jsonl`, bounded 500, newest-first) and **JobStore** (`~/.cozmo/jobs/*.json`, checkpointed executions).
...
- `ModelRuntime` (`cozmo/runtime/models/factory.py`) is a thin execution boundary: resolved identity → provider → LangChain model. Never selects/picks.
...
Extracted knowledge  → Brain VectorStore knowledge_items    [canonical KNOWLEDGE, typed]
                         + flat MemoryManager cozmo_memories [legacy fallback]
...
| # | Vision | Current | Gap |
|---|---|---|---|
| G1 | **LangGraph as orchestrator** | Hand-rolled ReAct loop + planner + retrieval executor + jobs | LangGraph unused entirely; all control flow in `CozmoRuntime` |
...
- **G1:** Introduce LangGraph state-graph for the agent loop (retrieval → reason → tool → reflect → answer), with the existing `CozmoRuntime` components (tools, sources, planner) as node callables. Keep current loop as fallback during migration.
...
1. **Keep LangChain for model integration** (`ChatOllama`/`ChatOpenAI` via `cozmo.providers` + `ModelRuntime`). It is correctly layered today.
...
| File | Change |
|---|---|
| `cozmo/runtime/runtime.py` | Wrap loop in LangGraph graph; keep streaming API |
| `cozmo/runtime/retrieval.py` | Wire `ResultMerger`; wire `EvidenceProcessor` into `execute_search` |
| `cozmo/runtime/retrieval_policy.py` / `result_merger.py` | Extend strategies for scenario/identity; unify ranking |
| `cozmo/code_indexer.py` | Chunking + mtime guard + dedup |
| `cozmo/memory/knowledge_index.py` | Wikilink parsing + backlinks + typed-column filters |
| `cozmo/brain/brain.py` | Markdown mirror on extract/learn; reconciliation |
| `cozmo/brain/reasoning/extraction.py` | Chat-capable fact extraction |
| `cozmo/evidence/` | Wire into runtime; chat-capable extractor |
| `cozmo/tools/file_ops.py` | Fix `KNOWLEDGE` path to configured `workspace.knowledge` |
| `cozmo/services/context.py` | Wire markdown mirror + new components |
...
| New: `cozmo/graph/` (LangGraph nodes/state), `cozmo/memory/wikilink.py`, tests |

## 13. What NOT to Change

- **`cozmo/providers` + `ModelRuntime` model construction** — correct layering, keep.
