...
// without scattering decisions across useCozmoChat and components.
...
export const notifyPolicy = {
  ...
  responseFailed(conversationTitle: string, conversationId: string): PolicyResult {
    return {
      draft: {
        ...
        message: `Cozmo hit an error in ${quote(conversationTitle)}`,
        conversationId,
        action: { type: 'openConversation', conversationId },
      },
  ...
  reconnected(): PolicyResult {
    return {
      draft: { severity: 'info', title: 'Reconnected', message: 'Cozmo is back online' },
      native: null,
    }
  },
}