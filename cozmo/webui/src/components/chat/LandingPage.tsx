...
export function LandingPage({
  ...
  const busyText = generating
    ? 'Cozmo is responding in this conversation.'
    : generatingElsewhereTitle
      ? `Cozmo is working in "${generatingElsewhereTitle}".`
      : activeRuns.length > 0
    ...
    <div className="flex-1 overflow-y-auto px-6 py-10">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-8">
          <section aria-label="Current activity">
            ...
            {!busy ? (
              <EmptyState
                ...
                description="Cozmo is idle — start a conversation or a background job."
              />
            ) : (
          ...
          <section aria-label="What Cozmo knows">
            <h2 className="font-medium text-sm text-base-100 mb-2.5">What Cozmo knows</h2>
            {knowledgePreview.length === 0 ? (
              <EmptyState
                compact
                icon={Brain}
                title="Still learning"
                description="Cozmo will build an understanding of your work as you talk."
              />
            ) : (
