...
* ``OpenAIRuntimeInventory`` — the OpenAI-compatible runtime is a hosted
  ...
  models explicitly configured in Cozmo config, each marked with the
...
# Ollama /api/show capability tokens mapped 1:1 to Cozmo capabilities.
...
def _capability_tokens_from_show(payload: dict) -> list[str]:
    """Map ``payload.capabilities`` tokens 1:1 to Cozmo capabilities.
