...
* Model selection  — Cozmo. The graph RECEIVES the already-bound runnable;
...
* Reflection       — Brain consolidation stays gated by Cozmo semantics via
...
from .state import RuntimeState, append_error, should_stop
...
class RuntimeWorkflowGraph:
    ...
    def _node_retrieve(self, state: dict) -> dict:
        """Snapshot the context Cozmo's retrieval pipeline already produced.
    ...
    def _node_act(self, state: dict) -> dict:
        ...
        if calls:
            ...
            events.append(("thinking", f"Running: {names}", detail, None))
    ...
    def _node_reflect(self, state: dict) -> dict:
        """Optional Brain consolidation hook — Cozmo-gated, default no-op."""
