"""ModelRuntime — thin execution boundary between Cozmo's model selection and LangChain.

Cozmo resolves WHICH model to use (``llm.workloads.<workload>.model``, verbatim,
...
delegating construction to the existing provider layer (``cozmo.providers``).
...
Responsibilities:
  ...
  * delegate construction to ``cozmo.providers`` (ChatOllama / ChatOpenAI)
...
log = logging.getLogger("cozmo.model_runtime")


@dataclass(frozen=True)
class ResolvedModel:
    """Model identity already resolved by Cozmo's selection system.

    The immutable hand-off between Cozmo model selection and LangChain model
